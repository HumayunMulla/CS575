...
int load=0, N=0, threadID=0,num_threads=0;
...
main(int argc, char *argv[])
{
	...
	num_threads = atoi(argv[4]); // to take for number of threads
...
/*  trapezoid func will be called each time to calcualte the area
    of each trapezoid by providing limits such as a & b.
*/
double trapezoid(double a, double b) {

    midpoint = (b + a) / 2; // calculate midpoint as it will be used to measure the height of trapezoid
    delx = b - a;   // width of the rectangle
    height = sin(midpoint) / midpoint; // height of the trapezoid also the slope of the line or the integral
    // area = delx * height;
	area = ((a+b)/2) * height;
	area = (area / num_threads)*2;

    return area; // return the calculated area for each trapezoid
